...
export default function QuoteForm({ settings }: { settings: Settings }) {
  ...
  async function onSubmit(data: FormValues) {
    setSubmitting(true);
    try {
      const deliverableObj = findDeliverable(data.deliverable);
      const timelineObj = findTimeline(data.timeline);

      const payload = {
        ...data,
        deliverableLabel: deliverableObj?.label ?? data.deliverable,
        timelineLabel: timelineObj?.label ?? data.timeline,
      ...
      const sheetRes = await fetch("/api/quote-sheets", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
      ...
      if (!sheetRes.ok) {
        ...
        toast.error("Failed to save quote", {
          description: err || "An error occurred while saving the quote data.",
        });
        return;
      }
